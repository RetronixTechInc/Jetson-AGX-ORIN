...
#include "hawk_owl_ar0231_tbls.h"
...
static const struct of_device_id ar0234_of_match[] = {
	{.compatible = "nvidia,ar0231_hawk_owl",},
	{ },
};
...
static int ar0234_hawk_link_check(struct ar0234 *priv)
{
	...
	if ((1 == priv->channel)) {
		max96712_read_reg_Dser(DES_ADDR, OWL_CHANNEL, 0x1A, &linkA);
		max96712_read_reg_Dser(DES_ADDR, OWL_CHANNEL, 0x0A, &linkB);
		max96712_read_reg_Dser(DES_ADDR, OWL_CHANNEL, 0x0B, &linkC);
		max96712_read_reg_Dser(DES_ADDR, OWL_CHANNEL, 0x0C, &linkD);

	} else if ((!priv->channel) || (2 == priv->sync_sensor_index)) {
		max96712_read_reg_Dser(DES_ADDR, HAWK_CHANNEL, 0x1A, &linkA);
		max96712_read_reg_Dser(DES_ADDR, HAWK_CHANNEL, 0x0A, &linkB);
		max96712_read_reg_Dser(DES_ADDR, HAWK_CHANNEL, 0x0B, &linkC);
		max96712_read_reg_Dser(DES_ADDR, HAWK_CHANNEL, 0x0C, &linkD);
	}
	pr_info("%s: linA=0x%x, linB=0x%x, linC=0x%x, linD=0x%x\n",__func__, linkA, linkB, linkC, linkD);
...
static int ar0234_hawk_owl_i2ctrans(struct ar0234 *priv)
{
	...
	link_count = ar0234_hawk_link_check(priv);
	if (link_count == 4)
...
static int hawk_owl_ser_i2c_trans(struct camera_common_data *s_data)
{
	...
	if(link_count == 4) {
		...
		pr_err("[%s-%d] ERROR, link num=0, owl_flag=%d\n", __func__, __LINE__, owl_flag);
	}
...
error:
	...
	err = camera_common_mclk_enable(s_data);
	if (err)
		dev_err(dev, "[RTX] Error %d turning on mclk\n", err);
...
static int ar0234_hawk_owl_deser_ser_program(struct ar0234 *priv)
{
...
#ifndef LEOPARD_AR0234
	err = ar0234_write_table(priv, mode_table[AR0231_DSER]);
#endif
	...
	if(link_count == 4) {
		if ((1 == priv->channel) && (!owl_flag)) {
...
#else
			err = ar0234_write_table(priv, mode_table[AR0231_QUADLINK_SER]);
#endif
			if (err)
			...
			owl_flag++;
			owl_links = QUAD_LINK;
		} else if ((!priv->channel) && !hawk_flag) {
...
#else
			err = ar0234_write_table(priv, mode_table[AR0231_TRIPLELINK_SER]);
#endif
			if (err)
				dev_err(dev,"%s: Failed to do OWL mode table..\n",__func__);
			else
...
#else
			err = ar0234_write_table(priv, mode_table[AR0231_DUALLINK_SER]);
#endif
			if (err)
				dev_err(dev,"%s: Failed to do OWL mode table..\n",__func__);
			else
...
#else
			err = ar0234_write_table(priv, mode_table[AR0231_SINGLELINK_SER]);
#endif
			if (err)
				dev_err(dev,"%s: Failed to do OWL mode table..\n",__func__);
			else
...
#endif
{
	...
	////// artie: to judge deserializer existed or not here //////
	if ((1 == priv->channel))
		max96712_read_reg_Dser(DES_ADDR, OWL_CHANNEL, 0x0D, &val);
	else
		max96712_read_reg_Dser(DES_ADDR, HAWK_CHANNEL, 0x0D, &val);
	
	if((val != MAX96724_ID) && (val != MAX96712_ID)) {
		dev_err(dev, "cannot find deserializer(ID=0x%x)\n", val);
		return -EINVAL;
	} else {
		dev_info(dev, "find deserializer: %s\n", val == MAX96724_ID ? "MAX96724" : "MAX96712");
	}
	///////////////////////////////////////////////////////////////
	...
	////// monitor link lock //////////////////////////////////
	msleep(60); // refer to tLOCK2 time in max96724 spec
	...
	pr_err("!!!!!!!!!!!monitor link_check close(%d)!!!!!!!!!!!\n", count);
	/////////////////////////////////////////////////////////
	...
	if (err) {
		dev_err(&client->dev,"[RTX] ERROR, Failed to program deser/ser\n");
		goto un_register;
	}
#ifdef LEOPARD_AR0234
	...
	dev_err(&client->dev, "Detected AR0231 sensor(%pOF)\n", node);
	return 0;
