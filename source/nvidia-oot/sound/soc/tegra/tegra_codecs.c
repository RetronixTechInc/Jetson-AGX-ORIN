...
#include <drivers-private/sound/soc/codecs/da7213.h>
...
static int tegra_machine_da7212_init(struct snd_soc_pcm_runtime *rtd)
{
	struct device *dev = rtd->card->dev;
	int err;

	err = snd_soc_dai_set_sysclk(rtd->dais[rtd->dai_link->num_cpus],
				     DA7213_CLKSRC_MCLK, 12288000,
				     SND_SOC_CLOCK_IN);
	if (err) {
		dev_err(dev, "failed to set da7212 sysclk!\n");
		return err;
	}
...
int tegra_codecs_init(struct snd_soc_card *card)
{
	...
	for (i = 0; i < card->num_links; i++) {
		...
		else if (strstr(dai_links[i].name, "da7212-playback"))
			dai_links[i].init = tegra_machine_da7212_init;
	}
